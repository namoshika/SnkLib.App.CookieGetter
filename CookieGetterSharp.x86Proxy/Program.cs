...
namespace Hal.CookieGetterSharp {
    ...
    class Program {
        static void Main(string[] args) {
            ...
            if(args.Length != 2) {
                Console.WriteLine("CookieGetterSharp.x86Proxy: CookieGetterSharpがx64向けアプリ下で動く際にx86環境下で呼び出す必要のあるAPIの代理実行をするプログラムです。");
                Console.ReadLine();
                return;
            }
            ...
            using(var pipeClient = new System.IO.Pipes.AnonymousPipeClientStream(System.IO.Pipes.PipeDirection.Out, args[1])) {
                //名前付きパイプでサービスを公開する
                var endpointUrl = new Uri(string.Format("net.pipe://localhost/CookieGetterSharp.x86Proxy/{0}/Service/", args[0]));
